...
// Linear interpolation between danger (#e85d5d) and success (#3fb950) based on sentiment 0–100
function sentimentToColor(sentiment: number, alpha = 1): string {
  const t = Math.max(0, Math.min(sentiment / 100, 1));
...
export function OutcomeHeatmapChart({ timeRange, onDrillDown }: OutcomeHeatmapChartProps) {
  ...
  return (
    <div className="widget insight-widget" style={{ overflowX: 'auto' }}>
      <h2 className="widget-title">Outcome heatmap: topic × intent</h2>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: '1rem', fontSize: 10, color: 'var(--text-muted)' }}>
        ...
        <div style={{
          ...
          height: 8,
          borderRadius: 4,
          background: `linear-gradient(to right, ${sentimentToColor(0)}, ${sentimentToColor(50)}, ${sentimentToColor(100)})`,
        }} />
