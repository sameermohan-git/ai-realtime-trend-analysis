...
export function TopicSentimentChart({ timeRange, onDrillDown }: TopicSentimentChartProps) {
  ...
  const colorFor = (avg: number) =>
    avg < 4 ? 'var(--danger)' : avg < 7 ? '#b8860b' : 'var(--success)';

  return (
    <div className="widget insight-widget">
      <WidgetHeading title="Topics by average member sentiment" infoText="Average member sentiment (0–10) per topic. Lower bars indicate topics that may need better handling or process changes." />
      <p className="widget-insight-caption">Lower bars = riskier topics; prioritize coaching and process.</p>
      <div className="widget-chart">
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data} layout="vertical" margin={{ left: 8, right: 24 }}>
            <XAxis type="number" domain={[0, 10]} stroke="var(--text-muted)" fontSize={11} />
