...
export function MemberAgentSentimentChart({ timeRange, onDrillDown }: MemberAgentSentimentChartProps) {
  ...
  return (
    <div className="widget insight-widget">
      <h2 className="widget-title">Member vs agent sentiment</h2>
      ...
      <div className="widget-chart">
        <ResponsiveContainer width="100%" height={260}>
          <ScatterChart margin={{ left: 8, right: 8, bottom: 8 }}>
            <XAxis type="number" dataKey="memberSentiment" name="Member" domain={[0, 100]} stroke="var(--text-muted)" fontSize={11} />
            <YAxis type="number" dataKey="agentSentiment" name="Agent" domain={[0, 100]} stroke="var(--text-muted)" fontSize={11} />
            ...
            <Scatter data={data} fill="var(--chart-1)">
              {data.map((entry, i) => (
                <Cell
                  key={i}
                  fill={entry.memberSentiment < 50 ? 'var(--danger)' : entry.memberSentiment < 70 ? '#b8860b' : 'var(--success)'}
