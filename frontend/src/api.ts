...
export interface CallSummary {
  ...
  summary?: string;
  actions?: CallAction[];
  /** Per-call QM check results (from transcript/QA) */
...
export interface CopilotChartConfig {
  ...
  dataKey: string;
  groupBy?: string;
  metric: 'intents' | 'topics' | 'sentiment' | 'calls' | 'volume-over-time';
}
...
export interface QmComplianceOverTimeBucket {
  label: string;
  overallRatePct: number;
  byCheck: { checkId: QmCheckId; ratePct: number }[];
}
...
export const api = {
  ...
  getActionsByTopic: (range: TimeRange) =>
    get<{ byTopic: ActionByTopicItem[] }>(`${API}/insights/actions-by-topic`, { range }),

  getTalkingPoints: (range: TimeRange) =>
    get<TalkingPointsResponse>(`${API}/insights/talking-points`, { range }),

  getQmCompliance: (range: TimeRange) =>
    get<{ checks: QmCheckResult[] }>(`${API}/insights/qm-compliance`, { range }),

  getQmComplianceOverTime: (range: TimeRange) =>
    get<{ buckets: QmComplianceOverTimeBucket[] }>(`${API}/insights/qm-compliance-over-time`, { range }),

  getComplianceSummary: (range: TimeRange) =>
    get<ComplianceSummary>(`${API}/dashboard/compliance-summary`, { range }),
  getSentimentSummary: (range: TimeRange) =>
    get<SentimentSummary>(`${API}/dashboard/sentiment-summary`, { range }),
  getRiskSummary: (range: TimeRange) =>
    get<RiskSummary>(`${API}/dashboard/risk-summary`, { range }),
  getTrendSummary: (range: TimeRange) =>
    get<TrendSummary>(`${API}/dashboard/trend-summary`, { range }),
};
